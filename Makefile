...
build: mod
	...
	go build -ldflags '-X main.GitTag=$(GIT_TAG) -X main.BuildTime=$(BUILD_DATE) -X main.GitCommit=$(COMMIT_HASH) -X main.GitAuthor="$(GIT_AUTHOR)"' ./cmd/lib/mock_main.go
...
test:
	@echo "Run unit tests"
	go test -test.short -cover -gcflags="all=-N -l" ./...
...
racecheck: mod
	go build -race -ldflags '-X main.GitTag=$(GIT_TAG) -X main.BuildTime=$(BUILD_DATE) -X main.GitCommit=$(COMMIT_HASH) -X main.GitAuthor="$(GIT_AUTHOR)"'  -o ${BIN_NAME} ./cmd/main.go
	mkdir -p output/bin
	cp -r ${BIN_NAME} output/bin
	cp -r conf output/
	cp -r scripts output/bin
...
adxtest:
	@echo "Run adx tests"
	cd ./internal/pkg/infrastructure/mvutil && go test -run TestIsStandardModel
